...
public class Settings extends JToolBar implements ItemListener
{
	...
	private DcelPanel dp;

	private JToggleButton[] buttons;

	private static final String TEXT_DRAW_CONNECTORS = "Connectors";

	public Settings(DcelPanel dp)
	{
		this.dp = dp;

		setFloatable(false);

		String as[] = { TEXT_DRAW_CONNECTORS };
		...
		buttons[0].setSelected(dp.getConfig().isDrawConnectors());

		ZoomInput zoom = new ZoomInput(dp);
		add(zoom);
	}
	...
	public void itemStateChanged(ItemEvent e)
	{
		...
		if (s.equals(TEXT_DRAW_CONNECTORS)) {
			dp.getConfig().setDrawConnectors(flag);
		}
		dp.settingsUpdated();
	}
